...
fully green (162 test files / 1434 tests, 80 Playwright tests with 8 skipped,
...
tv-android/          Google TV WebView shell around /tv (Java, Gradle); now in
                     an owner-only Google Play internal test. Has its own README
...
hosted database: pgTAP writes rows. A clean run is 26 suites / 710 assertions,
...
- A readout that settles late never renders its interim answers. The bowl
  ...
  loaded yet" is a wrong answer, not a placeholder. Remembered values are
  display only and never reach a draw.
