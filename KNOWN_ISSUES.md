...
Last reviewed: 2026-08-31 at commit `515cefd`.

## Confirmed bugs

### MB-001 — Returning a draw erases every participant's personal history

- **Severity:** P1
- **Status:** Confirmed bug
- **First observed:** `2233252`; reconfirmed after `b2522ba`
- **Invariant:** Moving a drawn title back to a bowl must not delete another
  person's durable watch history or their edited date and note.
- **Evidence:** `return_bowl_draw_to_bowl` authorizes one draw-capable caller,
  then deletes every `user_watch_events` row for the draw without filtering by
  user (`supabase/migrations/20260822120000_add_movie_comments.sql`, lines
  403–405 and 455–462).
- **Impact:** Any authorized return can erase personal records for all current
  participants. Already-deleted notes and dates cannot be reconstructed from
  the draw event.

**Implementation plan**

1. Ship a migration that redefines `return_bowl_draw_to_bowl` without deleting
   `user_watch_events`. `returned_at` already records the bowl-state reversal;
   personal watch events should remain immutable unless their owner edits or
   deletes them.
2. Add pgTAP coverage with two participants, including a locally edited watch
   date and note, and prove both records survive a return while the movie is
   restored exactly once.
3. Add or update the UI integration test to show the returned movie in the bowl
   while retained history remains visible.
4. Document that there is no general historical repair for records deleted
   before the migration. If database backups are available, assess restoration
   separately rather than guessing dates or notes.
5. Deploy the migration before the frontend that assumes retained history. The
   rollback is the prior function definition, though restoring the destructive
   delete should be reserved for an explicit product reversal.
...
No entries yet. Fixed and disproved findings stay here with their stable IDs so
future audits can distinguish recurrence from rediscovery.
