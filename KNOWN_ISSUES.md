...
Last reviewed: 2026-08-31 at commit `07445a2`.

## Confirmed bugs

### MB-001 — Returning an old draw erases every participant's personal history
...
- **Evidence:** `return_bowl_draw_to_bowl` authorizes one draw-capable caller,
  ...
  403–405 and 455–462).
- **Impact:** The useful accidental-draw undo has no time boundary. Returning a
  title days or months later erases personal records for every participant.
  Already-deleted titles and dates cannot be reconstructed from the group draw
  event.

**Decision history**

- August 31, 2026: returning through the watched strip was confirmed as an undo
  for a pick the group did not watch, not a rewatch feature. A two-hour window
  gives accidental and abandoned draws a complete group undo without inferring
  intent from per-user edits. Returns committed inside the window delete all
  generated participant history; later returns preserve it all. Rewatching uses
  the normal Add Movie flow. Full plan:
  `output/designs/tv-watch-history-details-and-safe-return.md`.
- August 31, 2026: the bounded RPC, preserve-all rollback, pgTAP coverage,
  matching phone/TV copy, fake backend, and browse-first TV detail experience
  were implemented in the repository. The issue intentionally remains
  **Confirmed bug** until the migration is applied and both sides of the
  production boundary are verified.

**Implementation plan**

1. Redefine `return_bowl_draw_to_bowl` to capture one server transaction
   timestamp, use it for `returned_at`, and delete generated bowl-draw history
   only when it is no more than two elapsed hours after `drawn_at`. Preserve the
   existing locked movie restore, permissions, capacity guard, and duplicate
   guard. No new history column is needed.
2. Add pgTAP coverage with two participants and prove a recent return cleans all
   generated rows, an older return preserves all rows, the exact two-hour
   boundary cleans, and the movie is restored exactly once. Cover repeated
   return, outsider, duplicate, capacity, and manual-history boundaries.
3. Update the fake backend and phone/TV integration tests to match the two-hour
   contract and its confirmation copy.
4. Document that there is no general historical repair for records deleted
   ...
   separately rather than guessing dates or titles.
5. Deploy the migration before the frontend that promises the two-hour rule.
   A rollback must fall back to preserving all personal history rather than
   restoring the unsafe unconditional delete.
