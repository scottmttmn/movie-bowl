...
- remove every personal history entry that draw created automatically.

Returning is only available within two hours of the draw. Past that the action
is refused, because "the group did not watch this pick" stops being a safe
reading of it, and a late return was the thing that let the surfaces disagree
about what the bowl had watched. Correcting the record after the window is a
separate job: see the owner-level delete tracked in `TODO.md`.
...
| Undo window | The draw was made no more than two hours ago | The return is available, and takes every participant's generated history with it |
| Past the undo window | More than two hours have elapsed since the draw | The return is refused; the pick stays in Watch History until an owner deletes it |
| Put back in bowl | The group did not watch the pick | Restores the slip, returns the draw event, and removes the generated personal history |
...
- A return disappears from Watch History on every surface. There is no
  post-window return to display, so no `Back in bowl` status exists.
...
- TV and the phone read one active-draw collection. Neither requests returned
  draws, so the two surfaces cannot disagree about what the bowl has watched.
...
- Past the window the return action is withheld and the screen says why. The
  database refuses a late return too, so an older client cannot make one.
