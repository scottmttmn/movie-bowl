...
direction settled August 31, 2026. Superseded September 3, 2026 on one point:
TV no longer keeps returned draws visible with a `Back in bowl` status. See
"Returned draws leave Watch History" at the end of this document.
...
- Every returned draw disappears from TV Watch History, the same as on the
  phone. (Superseded: this originally kept post-window returns visible with a
  `Back in bowl` status.)
...
- TV requested durable bowl history separately from the active-draw collection,
  so older returned draws remained browseable as `Back in bowl`. Superseded
  September 3, 2026: both surfaces now read one active-draw collection.
...
- TV Watch History drops every returned draw. (Superseded: older returns were
  originally kept visible and non-actionable.)
...
## Returned draws leave Watch History (September 3, 2026)

Keeping post-window returns on the TV strip put a `Back in bowl` badge on an
immutable record. `returned_at` says a draw *was* returned; it is never cleared,
and the return inserts a fresh `bowl_movies` row with no link back to the event.
So once that copy was drawn again or deleted, the badge went on asserting a
bowl membership that had moved on — while the phone, which never loaded returned
draws at all, showed something different for the same bowl.

TV now reads the same active-draw collection the phone does. A returned draw
leaves Watch History outright on both surfaces, and the two-hour window governs
only whether the generated `user_watch_events` rows survive. The badge, the
separate `watchHistory` collection, and the `includeReturnedHistory` option are
gone rather than reworded, because with one collection there is no returned draw
left to label.
