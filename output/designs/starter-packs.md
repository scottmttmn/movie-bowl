...
- **Committed: the Best Picture winners, as title and year.** TMDB carries no
  ...
  TMDB Content.
- **Resolved at run time into the database.** A resolver turns each definition
  into TMDB ids and writes `starter_pack_titles (pack_slug, tmdb_id)` with a
  `resolved_at`. Filmographies come from TMDB person credits, keeping feature
  films where the person directed or had a principal role and dropping entries
  under a minimum vote count -- otherwise cameos, uncredited parts, TV movies,
  documentaries and shorts fill the pack -- then splitting by release decade.
  Best Picture titles are matched by title and year. Anything ambiguous (two
  people with one name, two films with one title in one year) fails that pack
  loudly rather than guessing.
- **That table is a cache, and is refreshed inside six months.** The daily
  refresh cron already holds the TMDB token and the service role, so it
  re-resolves packs whose `resolved_at` is older than 90 days, a few per run
  inside its 60-second cap. A pack that cannot be refreshed is withheld from
  install rather than served stale. No new serverless function: the deployment
  is at Vercel Hobby's 12-function limit.
- **Titles and posters are never stored with the pack.** The install fetches
  each movie through the normal `/api/tmdb` details path, exactly like an
  ordinary add, and the resulting bowl rows are ordinary bowl rows.
- **The curation is a rule applied at display time,** the same kind of
  filtering the draw's rating and runtime filters already apply to TMDB data,
  rather than a published derivative list. That is our reading of §1.C, not a
  ruling.
...
1. The committed pack definitions and Best Picture list, the
   `starter_pack_titles` table with its `resolved_at`, and the resolver step in
   `api/cron/refresh-filter-metadata` that fills and refreshes it. Cheap, and it
