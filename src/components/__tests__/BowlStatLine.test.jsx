...
describe("BowlStatLine", () => {
  ...
  it("keeps an unscanned service list reachable", () => {
    const { onScanStreaming } = renderLine({
      poolStatus: DRAW_POOL_STATUS.ready,
      poolCount: 3,
    ...
    expect(onScanStreaming).toHaveBeenCalled();
  });

  it("stays quiet while a scan is running", () => {
    renderLine({
      poolStatus: DRAW_POOL_STATUS.ready,
      poolCount: 3,
    ...
    expect(pool()).toHaveTextContent("Drawing from 3");
    expect(screen.queryByText(/checking your services/i)).not.toBeInTheDocument();
  });
