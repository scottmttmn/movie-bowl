-- Safe rollback for the bounded return cleanup. Move this file into
-- supabase/migrations/ with a fresh timestamp to run it. Returning still
-- restores the bowl slip, but personal history is preserved at every age.
...
comment on function public.return_bowl_draw_to_bowl(uuid) is
  'Returns a draw to its bowl while preserving every participant personal history entry.';
