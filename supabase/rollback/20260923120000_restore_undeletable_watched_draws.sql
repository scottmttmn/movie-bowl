-- Withdraws owner removal from watched history: the function goes, and return
-- goes back to its 20260903120000 definition, which does not know about
-- removal.
--
-- The columns stay. The client filters on removed_at, so dropping it would
-- break every bowl read, and the draws already removed would reappear in the
-- watched lists of every member with no one having asked for that. Drop them
-- only alongside a client that no longer reads them.

begin;

drop function if exists public.remove_bowl_draw_from_history(uuid);
