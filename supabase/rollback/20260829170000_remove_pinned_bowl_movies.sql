-- Revert pinned bowl movies. Move this file into supabase/migrations/ with a
-- fresh timestamp to run it. Every saved pin is discarded; draw and rotation
-- behavior otherwise return to their pre-pin definitions.

begin;

drop function if exists public.set_own_bowl_movie_pin(uuid, boolean);

-- Remove the column dependency before dropping the column.
...
drop index if exists public.bowl_movies_one_pin_per_contributor;
