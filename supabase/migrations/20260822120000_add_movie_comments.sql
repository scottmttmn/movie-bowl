-- Add optional short comments to bowl slips and immutable watch snapshots.
-- Every draw path copies the slip comment through the private persistence
-- helper so ordinary and rotation draws cannot drift.
...
alter table public.bowl_movies
  add constraint bowl_movies_note_length_check
  check (note is null or char_length(note) <= 500);

alter table public.bowl_draw_events
  add constraint bowl_draw_events_note_length_check
  check (note is null or char_length(note) <= 500);

alter table public.user_watch_events
  add constraint user_watch_events_note_length_check
  check (note is null or char_length(note) <= 500);
...
revoke all on function public.update_own_bowl_movie_note(uuid, text)
from public, anon, authenticated;

grant execute on function public.update_own_bowl_movie_note(uuid, text)
to authenticated;

-- Keep the existing three-argument identity so PostgREST has no overload to
-- disambiguate. Link-created rows deliberately have no authenticated owner.
...
begin
  ...
  v_note := nullif(
    regexp_replace(coalesce(p_movie->>'note', ''), '^[[:space:]]+|[[:space:]]+$', '', 'g'),
    ''
  );
  if v_note is not null and char_length(v_note) > 500 then
    raise exception 'Comment must be 500 characters or fewer.'
...
drop function if exists public.update_user_watch_event(uuid, text, date, date);
...
comment on column public.bowl_draw_events.note is
  'Immutable comment snapshot copied when a bowl movie is drawn.';
