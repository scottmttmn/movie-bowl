-- update_own_bowl_movie_note and set_own_bowl_movie_pin already checked
-- authentication, row ownership, attribution, and undrawn state, but never
-- current bowl access. Both are security definer, so table RLS never
-- supplied that boundary either: a contributor removed from a bowl could
-- still edit their own comment or pin on a row they created, as long as
-- they still had its id. Losing access to a bowl must revoke mutations
-- inside that bowl even when the caller still knows a row uuid they
-- originally created, so both now also require current owner or member
-- access on the row's bowl. The generic "no longer available" error is
-- unchanged, so a former member still cannot tell a stale id from a real
-- one.
...
begin
  ...
  if not found
    or not (
      public.is_bowl_owner(v_bowl_id)
      or public.is_bowl_member(v_bowl_id)
    )
  then
...
comment on function public.update_own_bowl_movie_note(uuid, text) is
  'Updates only an authenticated contributor own undrawn non-link movie comment, and only while they still have current owner or member access to its bowl.';

comment on function public.set_own_bowl_movie_pin(uuid, boolean) is
  'Atomically moves or clears the signed-in contributor pin for one undrawn bowl movie, and only while they still have current owner or member access to its bowl.';
