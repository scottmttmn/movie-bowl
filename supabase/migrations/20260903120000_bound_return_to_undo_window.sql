-- Returning a movie means the group did not watch this pick. That reading only
-- holds close to the draw, so the two-hour window becomes the boundary on the
-- action itself rather than on its cleanup.
--
-- It previously governed only whether the draw's personal history rows were
-- deleted, which gave the same number two meanings: a return inside the window
-- was an undo, and a return outside it was something else the product never
-- named. Surfaces then disagreed about what that something else was, and one
-- of them began advertising returned draws as still being in the bowl.
--
-- With late returns refused, every permitted return is an undo, so the personal
-- cleanup is unconditional again. Correcting history after the window is a
-- separate job -- see TODO.md MB-014 -- and putting the title back for another
-- viewing is Add Movie, which leaves the earlier draw and everyone's history
-- intact.
